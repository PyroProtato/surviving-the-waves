...
func _process(_delta: float) -> void:
	...
	if Input.is_action_just_pressed("destroy") and game_manager.selected_item() == "pickaxe":
	...
	#Placing blocks
	if Input.is_action_just_pressed("place") and item_database.blocks.has(game_manager.selected_item()):
